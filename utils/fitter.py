...
from utils.funcs import calculate_decimal, calculate_zero, convert_function, evaluate_polynomial, solve_lse, find_residual_func, adaptive_sampling, FLPsimFXP, overflowR, underflowR, piece_prediction
...
class PolyK():
    ...
    def __init__(self, k, coeff=None, scale = None, breaks = None, decimal = None):
        self.k = k
        self.coeff = coeff
    ...
    def fit (self, a, b, func, f, n, ms=1000, derivative_flag=False, method="Cheby", zero_mask=None):
...
class MPCApproximation:
    
    def __init__(self, k, coeff, decimal, scale, breaks, n, f):
        self.n, self.f = n, f   
        # self.poly = PolyK (k, coeff = coeff,  scale = scale, breaks = breaks, decimal = decimal)
        
        self.coeffA, self.scale_list, self.breaks = torch.from_numpy(coeff).float() , torch.from_numpy(scale).float(), torch.from_numpy(breaks).float()
        
        # self.coeffA, self.scale_list, self.breaks = coeff , scale, breaks
        
        
    def to_device(self, device):
        self.coeffA = self.coeffA.to(device)
        self.scale_list = self.scale_list.to(device)
        self.breaks = self.breaks.to(device)
        return 
    ...
    def predict(self, x):
        ...
        # print(x.shape, self.poly.breaks.shape)
        # xb = x.numpy().flatten()
        xb = x.flatten()

        xb = piece_prediction(xb[:, np.newaxis], self.coeffA, self.breaks, self.scale_list, self.f, self.n)
        # xb = self.poly.predict(xb, self.n, self.f)
        x = xb.reshape(x.shape)
