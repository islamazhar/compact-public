...
def overflowR (x, n, f):
    ...
    if isinstance(x, np.ndarray):
        ...
        x = x.flatten()
        sign = torch.sign(x)
        overflow_flag = torch.abs(x) > 2**(n - f - 1)
        x[overflow_flag] = (sign * 2**(n - f - 1))[overflow_flag]
...
def underflowR(x, f):
    ...
    if isinstance(x, np.ndarray):
        ...
        x = x.flatten()
        sign = torch.sign(x)
        underflow_flag = torch.abs(x) < 2**(-f)
        x[underflow_flag] = (sign * 2**(-f))[underflow_flag]
        x = torch.reshape(x, shape)
    else:
        raise NotImplementedError("Not support type " + str(type(x)))
...
def FLPsimFXP(x, n, f):
    ...
    if isinstance(x, np.ndarray) or isinstance(x, float):
        ...
        x = overflowR(x, n, f)
        x = underflowR(x, f)
        x = torch.round(x, decimals=decimal)
        return x
...
def sampled_error(y_true, y_pred, f, zero_mask=None, method="relative_error"):
    ...
    elif method == "density_error": #todo
        pass
...
def calculate_kx(x, k):
    ...
    in_set = {0, 1}
    # print("shape numpy = ", np.tile(x, (k, 1)).shape)
    if isinstance(x, torch.Tensor):
        items = torch.tile(x, (k, 1))
        # items = torch.from_numpy(np.transpose(items.numpy()))
        # print(items.shape)
        items = torch.transpose(items, 0, 1) # axis: range(a.ndim)[::-1]
    else:
    ...
    # print("items out shape = ", items.shape)
...
"""
    add when x < 10 or x > 10 return either  0 or 1
"""
...
def piece_prediction(x, coeffA, breaks, scale_list, f, n):
    ...
    if isinstance(x, np.ndarray):
        ...
        x = np.squeeze(x)
        tmp = calculate_kx(x, len(coeff[0]))
        # print("After calculate shape", tmp.shape)
        x_items = FLPsimFXP( tmp * coeff, n, f)
        ...
        x_index = x >= breaks[:-1]
        x_index = torch.sum(x_index, axis=1) - 1  # in cipher-text -> get_last_one.
        coeff = coeffA[x_index]  # select coeff.
        scale = scale_list[x_index]  # select the scale factor
        x = torch.squeeze(x)
        tmp = calculate_kx(x, len(coeff[0])) 
...
def adaptive_sampling(start, end, func, f, ms, zero_mask=None):
    ...
    # Sampling method
    ms = min(ms, input_indis) # step size
    x = np.unique(np.linspace(start, end, ms).round(decimal))
    # print("Before y = func(x).round(decimal)")
    # print(f"x type {type(x)}")
    # print(decimal)
    # print(func)
    y = func(x).round(decimal)
    # print("After y = func(x).round(decimal)")
